"""
Walmart BSR 페이지 크롤러 (DrissionPage 기반)
...
class WalmartBSRCrawler(WalmartBaseCrawler):
    """
    Walmart BSR 페이지 크롤러 (DrissionPage 기반)
    """
    ...
    def initialize(self):
        ...
        self.db_item_map = self.build_db_item_cache('wmart_hhp_product_list')
    ...
    def save_products(self, products):
        ...
        print()
        print(f"[Page {page_number} 통계] 시도: {page_attempted} | 중복: {page_dup} | 키워드필터: {page_keyword} | Preowned: {page_preowned} | URL없음: {page_url_missing} | 비제품: {page_nonprod} | UPDATE: {update_count} | INSERT: {insert_count}")
        print()
