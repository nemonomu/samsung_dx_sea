...
================================================================================
# 처음부터 실행
python wmart_hhp_crawl_test.py

# 특정 단계부터 재시작
python wmart_hhp_crawl_test.py --resume-from detail --batch-id w_20250123_143045
python wmart_hhp_crawl_test.py --resume-from bsr --batch-id w_20250123_143045
...
from walmart.hhp.wmart_hhp_main import WalmartMainCrawler
from walmart.hhp.wmart_hhp_bsr import WalmartBSRCrawler
from walmart.hhp.wmart_hhp_dt import WalmartDetailCrawler
from common.base_crawler import BaseCrawler
from common.alert_hhp_monitor import send_crawl_alert


class WalmartIntegratedCrawlerTest:
    """Walmart 통합 크롤러 (테스트용)"""
    ...
    def run(self):
        ...
        print("\n" + "="*60)
        print("Walmart Integrated Crawler (Test)")
        print("="*60)
        ...
        try:
            ...
            if not self.resume_from or self.resume_from == 'main':
                ...
                try:
                    success = WalmartMainCrawler(test_mode=True, batch_id=self.batch_id).run()
            ...
            if not self.resume_from or self.resume_from in ['main', 'bsr']:
                ...
                stage_start = time.time()
                try:
                    success = WalmartBSRCrawler(test_mode=True, batch_id=self.batch_id).run()
            ...
            stage_start = time.time()
            try:
                success = WalmartDetailCrawler(batch_id=self.batch_id, test_mode=True).run()
...
def main():
    """테스트용 통합 크롤러 진입점"""
    parser = argparse.ArgumentParser(description='Walmart HHP Integrated Crawler (Test)')
    ...
    crawler = WalmartIntegratedCrawlerTest(resume_from=args.resume_from, batch_id=args.batch_id)
    success = crawler.run()
