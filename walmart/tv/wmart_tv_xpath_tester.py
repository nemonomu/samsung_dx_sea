"""
Walmart TV XPath Tester (DrissionPage 버전)
...
사용법
================================================================================
python walmart/tv/wmart_tv_xpath_tester.py
...
DETAIL_XPATH_LIST = {
    ...
    # ===== 1-1단계: 가격 (페이지 로드 직후) =====
    # final_sku_price → not_available → see_more_seller_options 순 fallback
    # 'final_sku_price': '//span[@itemprop="price" and @data-seo-id="hero-price"]/text()',
    # 'final_sku_price_v2': '//span[@itemprop="price" and @data-seo-id="hero-price" and not(ancestor::div[contains(@class, "sticky-buy-box-column")])]/text()',
    ...
    # 'original_sku_price_v2': '//span[@data-seo-id="strike-through-price" and not(ancestor::div[contains(@class, "sticky-buy-box-column")])]/text()',
    'savings': "//div[contains(@class, 'buy-box-column') and not(contains(@class, 'sticky-buy-box-column'))]//div[@data-testid='dollar-saving']//*[starts-with(normalize-space(), '$')]/text()",
    ...
    # ===== 2단계: TV 스펙 모달은 별도 MODAL_XPATH_LIST에서 테스트 (모달 열기 후만 유효) =====
    ...
    # 상단 header_rating 추출 실패 시에만 사용 (개별 XPath fallback)
    # review_section: 스크롤 트리거용 — "Customer ratings & reviews" 헤더가 실제로 렌더된 시점 매칭
    ...
    # detail_page_count_of_reviews: 상세페이지(2차) — count_of_reviews가 K/M일 때 정확한 정수 추출
    # "Showing 1-3 of 10 reviews" h3 → 파서가 'of N reviews'의 N을 뽑음
    # 주의: count_of_reviews_* 패턴을 피해야 함 (chain 자동 스캔 회피)
    # 'detail_page_count_of_reviews': "//section[@id='item-review-section']//h3[contains(text(), 'Showing') and contains(text(), 'reviews')]/text()",
    # review_page_count_of_reviews: 리뷰페이지(3차) 전용 — 상세페이지와 별도 키로 분리
    # 주의: detail_page_count_of_reviews_* 패턴 피해서 chain fallback에 묶이지 않게 (페이지가 달라 의미상 fallback 아님)
    # 'review_page_count_of_reviews': '',
    # reviews_button: "View all reviews" — 카운트 괄호 '(' 없는 버튼
    # 'reviews_button': "//section[@id='item-review-section']//button[contains(text(), 'View all reviews') and not(contains(text(), '('))]",
    # reviews_button_fallback: "View all reviews (1.9만)" — 카운트 괄호 '(' 있는 버튼
    ...
    #'review_pagination': '//a[@data-automation-id="page-number" and text()="1"]',  # TODO: 테스트용 — 원본은 text()="{page_num}"
}
...
MODAL_XPATH_LIST = {
    ...
    # Screen size — 모달 안 (sku와 동일 구조 가정, 실제 HTML 확인 후 보정 필요)
    'screen_size': "//div[contains(@class, 'ModalPortal')]//h3[contains(text(), 'Screen size')]/following-sibling::div/span/text()",
    'modal_close_button': "//button[@aria-label='Close dialog']",
}
...
# ================================================================================
# 기본 사용할 XPath
LIST_BASE_CONTAINER = '//div[contains(@class, "mb0") and contains(@class, "ph0-xl") and .//h3[@data-automation-id="product-title"]]'

# 리스트 아이템 내 필드 XPath (상대 경로 - .// 로 시작)
# 비어있는 항목은 dx_xpath_selectors DB에 등록된 값으로 채워서 테스트하면 됨
LIST_FIELD_XPATHS = {
    # 'product_url': './/a[contains(@class, "w-100 h-100 z-1")]/@href',
    # 'retailer_sku_name': './/h3[@data-automation-id="product-title"]',
    # 'offer': './/span[contains(text(), "free offers")]',
    # 'pick_up_availability': './/div[@class="ff-text-wrapper" and contains(., "Free pickup")]//span[@class="b"]',
    # 'fastest_delivery': './/div[@class="ff-text-wrapper" and contains(., "Free shipping")]//span[@class="b"]',
    #'delivery_availability': ".//div[contains(@class, 'ff-text-wrapper') and contains(., 'Delivery')]//span[contains(@class, 'b')]/text()",
    # 'sku_status_1': './/div[text()="Sponsored"]',
    # 'sku_status_2': './/i[contains(@class, "ld-Rollback")]/following-sibling::span',
    #'available_quantity_for_purchase': './/div[@data-test-id="gpt-low-inventory-container"]//span[contains(., "Only") and contains(., "left")]',
    # 'inventory_status': './/div[@data-test-id="gpt-low-inventory-container"]//span[contains(., "Low stock")]/text()',


    "pick_up_availability": ".//*[@data-testid='badgeTagComponent'][contains(translate(normalize-space(.), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'pickup')]//text()[normalize-space() and not(ancestor::*[contains(@class, 'ld_Ec')]) and not(contains(translate(normalize-space(.), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'pickup'))]",
    
    "fastest_delivery": ".//*[@data-testid='badgeTagComponent'][contains(translate(normalize-space(.), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'shipping')]//text()[normalize-space() and not(ancestor::*[contains(@class, 'ld_Ec')]) and not(contains(translate(normalize-space(.), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'shipping'))]",
    
    "delivery_availability": ".//*[@data-testid='badgeTagComponent'][contains(translate(normalize-space(.), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'delivery')]//text()[normalize-space() and not(ancestor::*[contains(@class, 'ld_Ec')]) and not(contains(translate(normalize-space(.), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'delivery'))]",
    
    "inventory_status": ".//*[@data-testid='badgeTagComponent'][contains(translate(normalize-space(.), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'low stock')]",
    
    "available_quantity_for_purchase": ".//*[contains(translate(normalize-space(.), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'only') and contains(translate(normalize-space(.), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'left') and not(.//*[contains(translate(normalize-space(.), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'only') and contains(translate(normalize-space(.), 'ABCDEFGHIJKLMNOPQRSTUVWXYZ', 'abcdefghijklmnopqrstuvwxyz'), 'left')])]",
}


class WalmartTVXPathTester(WalmartBaseCrawler):
    """Walmart TV XPath 테스터 (DrissionPage 버전)"""
    ...
    def normalize_xpath_results(self, results):
        """lxml XPath 결과를 출력용 리스트로 정규화"""
        if isinstance(results, list):
            return results
        if results is None:
            return []
        value = str(results).strip()
        return [value] if value else []

    def format_list_field_value(self, field_name, value):
        """리스트 필드별 테스트 출력값 보정"""
        if field_name == 'available_quantity_for_purchase':
            return self.convert_first_number(value)
        return value
    ...
    def test_detail_page(self, tree, xpath_dict=None, label='상세페이지 XPath'):
        ...
        for field_name, xpath in xpath_dict.items():
            ...
            print(f"  XPath: {xpath}")
            try:
                results = self.normalize_xpath_results(tree.xpath(xpath))
                if results:
                    found_count += 1
    ...
    def test_list_page(self, tree):
        ...
        for idx, item in enumerate(base_containers, 1):
            ...
            for field_name, xpath in LIST_FIELD_XPATHS.items():
                ...
                print(f"    XPath: {xpath}")
                try:
                    results = self.normalize_xpath_results(item.xpath(xpath))
                    if results:
                        print(f"    매칭 개수: {len(results)}개")
                        values = self.extract_all_values(results)
                        for i, value in enumerate(values, 1):
                            value = self.format_list_field_value(field_name, value)
                            if not value:
                                value = "(빈값)"
                            elif len(value) > 60:
                                value = value[:60] + "..."
        ...
        for field_name, field_xpath in LIST_FIELD_XPATHS.items():
            ...
            for item in base_containers:
                try:
                    results = self.normalize_xpath_results(item.xpath(field_xpath))
                    if results:
                        success_count += 1
                except:
                    pass
    ...
    def click_spec_modal(self):
        """TV 스펙 모달 열기 — spec_toggle_button → more_details_button"""
    ...
    def run(self):
        ...
        try:
            print("\n" + "=" * 70)
            print("Walmart TV XPath Tester (DrissionPage)")
            print("=" * 70)
            print("\n[XPath 목록]")
