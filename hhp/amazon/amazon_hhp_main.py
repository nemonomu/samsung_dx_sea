"""
Amazon HHP Main 페이지 크롤러 (DrissionPage 기반)
"""
...
class AmazonMainCrawler(AmazonBaseCrawler):
            ...
            for idx, item in enumerate(base_containers, 1):
                try:
                    product_data = {
                        ...
                        'delivery_availability': self.extract_delivery_field(item, 'delivery_availability'),
    ...
    def save_products(self, products):
        ...
        print()
        print(f"[Page {page_number} 통계] 시도: {page_attempted} | 중복: {page_dup} | 키워드필터: {page_keyword} | URL없음: {page_url_missing} | 비제품: {page_nonprod} | 저장: {len(unique_products)}")
        print()
