...
class BestBuyTrendCrawler(BestBuyBaseCrawler):
    """
    BestBuy HHP Trend 페이지 크롤러
    """
    ...
    def run(self):
        ...
        try:
            ...
            print(f"  키워드제외: {self.stats['keyword_filtered']}")
    ...
    def save_products(self, products):
        ...
        print()
        print(f"[Page {page_number} 통계] 시도: {page_attempted} | 중복: {page_dup} | 키워드제외: {page_keyword} | OpenBox: {page_openbox} | URL없음: {page_url_missing} | 비제품: {page_nonprod} | UPDATE: {update_count} | INSERT: {insert_count}")
        print()
...
def main():
    """개별 실행 진입점 (테스트 모드)"""
    parser = argparse.ArgumentParser(description='BestBuy HHP Trend Crawler')
