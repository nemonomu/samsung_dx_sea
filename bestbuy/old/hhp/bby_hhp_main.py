...
- 테스트 모드: test_count 설정값만큼만 수집
- 운영 모드: max_products 설정값만큼만 수집
...
class BestBuyMainCrawler(BestBuyBaseCrawler):
    """
    BestBuy HHP Main 페이지 크롤러 (DrissionPage 기반)
    """
    ...
    def save_products(self, products):
        ...
        print()
        print(f"[Page {page_number} 통계] 시도: {page_attempted} | 중복: {page_dup} | 키워드필터: {page_keyword} | OpenBox: {page_openbox} | URL없음: {page_url_missing} | 비제품: {page_nonprod} | 저장: {len(unique_products)}")
        print()
