...
from bestbuy.old.hhp.bby_hhp_main import BestBuyMainCrawler
from bestbuy.old.hhp.bby_hhp_bsr import BestBuyBSRCrawler
from bestbuy.old.hhp.bby_hhp_trend import BestBuyTrendCrawler
from bestbuy.old.hhp.bby_hhp_dt import BestBuyDetailCrawler
...
class BestBuyIntegratedCrawler:
    ...
    def finish(self, crawl_results):
        """결과 출력, 알림 발송, 로그 종료를 공통 처리한다."""
    ...
    def run(self):
        ...
        try:
            ...
            if not self.resume_from or self.resume_from == 'main':
                ...
                if not crawl_results['main']['success']:
                    print("[STOP] Main Crawler failed - stopping remaining steps")
                    crawl_results['bsr'] = 'skipped'
                    crawl_results['trend'] = 'skipped'
                    crawl_results['detail'] = 'skipped'
                    return self.finish(crawl_results)
            else:
            ...
            if not self.resume_from or self.resume_from in ['main', 'bsr']:
                ...
                if not crawl_results['bsr']['success']:
                    print("[STOP] BSR Crawler failed - stopping remaining steps")
                    crawl_results['trend'] = 'skipped'
                    crawl_results['detail'] = 'skipped'
                    return self.finish(crawl_results)
            else:
            ...
            if not self.resume_from or self.resume_from in ['main', 'bsr', 'trend']:
                ...
                if not crawl_results['trend']['success']:
                    print("[STOP] Trend Crawler failed - stopping remaining steps")
                    crawl_results['detail'] = 'skipped'
                    return self.finish(crawl_results)
            else:
                crawl_results['trend'] = 'skipped'
