...
STEP 3. Promotion - 프로모션 페이지에서 제품 목록 수집 (test_count 설정값)
STEP 4. Trend     - 트렌드 페이지에서 제품 목록 수집 (test_count 설정값)
...
================================================================================
# 처음부터 실행
python bby_tv_crawl_test.py

# 특정 단계부터 재시작
python bby_tv_crawl_test.py --resume-from detail --batch-id b_20250123_143045
python bby_tv_crawl_test.py --resume-from bsr --batch-id b_20250123_143045

================================================================================
저장 테이블
================================================================================
- Main/BSR/Promotion/Trend → bby_tv_product_list (제품 목록)
- Detail                   → test_tv_retail_com (상세 정보 + 리뷰)
"""
...
from bestbuy.old.tv.bby_tv_main import BestBuyTVMainCrawler
from bestbuy.old.tv.bby_tv_bsr import BestBuyTVBSRCrawler
from bestbuy.old.tv.bby_tv_pmt import BestBuyTVPromotionCrawler
from bestbuy.old.tv.bby_tv_trend import BestBuyTVTrendCrawler
from bestbuy.old.tv.bby_tv_dt import BestBuyTVDetailCrawler
...
class BestBuyTVIntegratedCrawlerTest:
    """BestBuy TV 통합 크롤러 (테스트용)"""
    ...
    def run(self):
        ...
        print("\n" + "="*60)
        print("BestBuy TV Integrated Crawler (Test)")
        print("="*60)
        ...
        try:
            ...
            if self._should_run('main'):
                ...
                try:
                    success = BestBuyTVMainCrawler(test_mode=True, batch_id=self.batch_id).run()
            ...
            if stop_pipeline:
                ...
                stage_start = time.time()
                try:
                    success = BestBuyTVBSRCrawler(test_mode=True, batch_id=self.batch_id).run()
            ...
            if stop_pipeline:
                ...
                stage_start = time.time()
                try:
                    success = BestBuyTVPromotionCrawler(test_mode=True, batch_id=self.batch_id).run()
            ...
            if stop_pipeline:
                ...
                stage_start = time.time()
                try:
                    success = BestBuyTVTrendCrawler(test_mode=True, batch_id=self.batch_id).run()
            ...
            if stop_pipeline:
                ...
                stage_start = time.time()
                try:
                    success = BestBuyTVDetailCrawler(batch_id=self.batch_id, test_mode=True).run()
            ...
            print(f"\n[ERROR] BestBuy TV Integrated Crawler (Test Mode) failed: {e}")
            traceback.print_exc()
...
def main():
    """테스트용 통합 크롤러 진입점"""
    parser = argparse.ArgumentParser(description='BestBuy TV Integrated Crawler (Test Mode)')
    ...
    crawler = BestBuyTVIntegratedCrawlerTest(resume_from=args.resume_from, batch_id=args.batch_id)
    success = crawler.run()
