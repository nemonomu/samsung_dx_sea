"""
BestBuy TV Detail Update 크롤러

================================================================================
실행 모드
================================================================================
- 개별 실행: python bby_tv_dt_update.py --batch-id <batch_id> [--mode 1|2] [--start-id N]
...
주요 기능
================================================================================
- tv_retail_com 테이블에서 조건에 맞는 레코드 조회 (디테일 수집 실패/미수집 건)
...
class BestBuyTVDetailUpdateCrawler(BestBuyTVDetailCrawler):
    """
    BestBuy TV Detail Update 크롤러 (item IS NULL 재수집)
    """
    ...
    def load_product_list(self):
        """tv_retail_com 테이블에서 조건에 맞는 제품 조회 (UPDATE 대상)"""
        try:
            ...
            for row in rows:
                product = {
                    ...
                    'sku_status': row[7],
                    'promotion_type': row[8],
                    'main_rank': row[9],
                    'bsr_rank': row[10],
                    'trend_rank': row[11],
                    'promotion_position': row[12],
                    'product_url': row[13],
                    'calendar_week': row[14]
                }
                product_list.append(product)

            print(f"[INFO] Loaded {len(product_list)} products ({mode_desc}) from {self.table_name}")
            return product_list
    ...
    def update_retail_com(self, product):
        ...
        try:
            ...
            # crawl_datetime은 항상 업데이트
            updates.append("crawl_datetime = %s")
            params.append(
                product.get('crawl_datetime')
    ...
    def restart_browser(self):
        """브라우저 재시작 (메모리 정리)"""
        return super().restart_browser()

    def run(self):
        """실행: initialize() → load_product_list() → 제품별 crawl_detail() → update_retail_com() → 리소스 정리"""
        try:
            # 로깅 시작
            self.start_logging(self.batch_id or 'tv_dt_update')
            ...
            # 첫 제품 전 메인 페이지 방문 (세션 자연스럽게 시작)
            try:
                print("[INFO] BestBuy 메인 페이지 방문 중...")
                self.page.get('https://www.bestbuy.com')
                time.sleep(random.uniform(3, 5))
                print("[INFO] 메인 페이지 로드 완료")
            except Exception as e:
                print(f"[WARNING] 메인 페이지 방문 실패: {e}")
            ...
            for i, product in enumerate(product_list, 1):
                try:
                    ...
                    row_id = product.get('id')
                    print(f"\n{'='*70}")
                    print(f"[{i}/{len(product_list)}] (id={row_id}) {sku_name[:60]}")
                    print(f"{'='*70}")
            ...
            print(f"[DONE] Processed: {len(product_list)}, Updated: {total_updated}, Table: {self.table_name}, batch_id: {self.batch_id}")
            return True
...
def fetch_today_batch_ids(table_name):
    """오늘 날짜의 BestBuy TV batch_id 목록을 DB에서 조회"""
    import psycopg2
...
def main():
    """개별 실행 진입점"""
    import argparse
    parser = argparse.ArgumentParser(description='BestBuy TV Detail Update Crawler')
    ...
    if not batch_id:
        ...
        if today_batch_ids:
            print(f"\n오늘({datetime.now().strftime('%Y-%m-%d')}) BestBuy TV batch_id 목록:")
            ...
            print(f"오늘({datetime.now().strftime('%Y-%m-%d')}) BestBuy TV batch_id가 없습니다.")
    ...
    crawler = BestBuyTVDetailUpdateCrawler(batch_id=batch_id, start_id=start_id, mode=mode, test_mode=test_mode)
