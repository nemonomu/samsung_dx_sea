"""
Amazon HHP Integrated Crawler (Production)
...
from amazon.hhp.amazon_hhp_main import AmazonMainCrawler
from amazon.hhp.amazon_hhp_bsr import AmazonBSRCrawler
...
class AmazonIntegratedCrawler:
    """Amazon integrated crawler for production."""
    ...
    def run(self):
        """Run the integrated crawler. Returns True when at least one stage succeeds."""
        ...
        print("\n" + "="*60)
        print("Amazon Integrated Crawler (Production)")
        print("="*60)
        ...
        try:
            ...
            if (not self.resume_from or self.resume_from in ['main', 'bsr', 'detail']) and main_ok and bsr_ok:
                ...
                stage_start = time.time()
                try:
                    success = AmazonDetailCrawler(batch_id=self.batch_id, test_mode=False).run()
...
def main():
    """Production integrated crawler entry point."""
    ...
    crawler = AmazonIntegratedCrawler(resume_from=args.resume_from, batch_id=args.batch_id)
    success = crawler.run()
